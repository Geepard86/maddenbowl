...
(function (global) {
  ...
  function getUpcomingMatches(state, opts = {}) {
    ...
    // BUGFIX (dynamische Kickoff-Zeiten): `time` kommt jetzt aus dem
    // Live-Zeitplan (computeLiveKickoffSchedule) statt aus der rein
    // statischen Berechnung — berücksichtigt also, wenn ein Feld dem
    // ursprünglichen Zeitplan schon voraus/hinterher ist.
    const live = computeLiveKickoffSchedule(state);
    const all = [];
    ...
    (state.matches || []).forEach((m, idx) => {
      ...
      const matchId = `g-${idx}`;
      const liveTime = live.byMatchId.get(matchId);
      all.push({
        kind: "group", matchId,
        homeName: p1.name, awayName: p2.name, homeTeam: p1.team, awayTeam: p2.team,
        time: liveTime ? liveTime.time : getGroupMatchTime(state, m), stadium, phase: "Regular Season",
      });
    });

    (state.playoffMatches || []).forEach((m, idx) => {
      ...
      const matchId = `p-${m.id}`;
      const liveTime = live.byMatchId.get(matchId);
      all.push({
        kind: "playoff", matchId,
        homeName: m.p1.name, awayName: m.p2.name, homeTeam: m.p1.team, awayTeam: m.p2.team,
        time: liveTime ? liveTime.time : (computePlayoffTimes(state).byId[m.id] || computePlayoffTimes(state).groupEndTime), stadium, phase: m.phase,
      });
    });
  ...
  // ======================================================================
  // LIVE-ZEITPLAN (dynamische Kickoff-Zeiten)
  // -------------------------------------------------------------------------
  // Feste Kickoff-Uhrzeiten (rein rechnerisch aus config.start + Dauer je
  // Slot) passen erfahrungsgemäß nicht — Spiele dauern mal länger, mal
  // kürzer. Pro Feld (1/2, unabhängig voneinander, da parallel gespielt)
  // wird deshalb ein "echter Anker" mitgeführt: sobald das Ergebnis des
  // VORHERIGEN Spiels auf demselben Feld eingetragen wird (match.finishedAt),
  // gilt für das nächste Spiel auf diesem Feld
  //   Kickoff = finishedAt + KICKOFF_LOCK_BUFFER_MINUTES.
  // Für das jeweils erste Spiel eines Feldes (noch kein Vorgänger-Ergebnis)
  // sowie für alle weiter in der Zukunft liegenden Spiele, solange das davor
  // noch offen ist, wird die Planzeit fortgeschrieben (ausgehend vom zuletzt
  // bekannten echten Anker plus den üblichen Slot-Dauern) — so bleibt der
  // zuletzt gemessene Vorsprung/Rückstand für die Schätzung erhalten, statt
  // wieder auf die ursprüngliche statische Zeit zurückzuspringen.
  //
  // Liefert:
  //   byMatchId: Map(matchId -> { time: "HH:MM", isReal, plannedTime: "HH:MM" })
  //   drift: { 1: {minutes, isReal}, 2: {minutes, isReal} }
  //     minutes > 0 => Feld läuft diese Anzahl Minuten HINTER dem
  //     ursprünglichen Zeitplan, < 0 => diese Anzahl Minuten DAVOR.
  // matchId-Format identisch zu getUpcomingMatches(): `g-<index>` / `p-<id>`.
  const KICKOFF_LOCK_BUFFER_MINUTES = 5;
  ...
  function computeLiveKickoffSchedule(state) {
    const times = computePlayoffTimes(state);

    // Einheitliche, chronologisch sortierte Sequenz aus Gruppen- + Playoff-
    // Spielen, je mit Feld (1/2), Ist-Zeitstempel (finishedAt, falls vorhanden)
    // und statischer Planzeit. Freilose haben keinen echten Termin und werden
    // ausgelassen (wie schon in getUpcomingMatches()).
    const seq = [];
    (state.matches || []).forEach((m, idx) => {
      const slot = (state.slots || []).find((s) => s.includes(m));
      const field = (slot && slot.indexOf(m) === 1) ? 2 : 1;
      seq.push({
        matchId: `g-${idx}`, field, match: m,
        plannedTime: getGroupMatchTime(state, m),
        durationMin: (state.config.durG || 0) + (state.config.durB || 0),
      });
    });
    (state.playoffMatches || []).forEach((m, idx) => {
      if (!m.p1 || !m.p2 || m.p2.id === -1) return;
      const field = (idx % 2 === 0) ? 1 : 2;
      seq.push({
        matchId: `p-${m.id}`, field, match: m,
        plannedTime: times.byId[m.id] || times.groupEndTime,
        durationMin: (state.config.durP || 0) + (state.config.durB || 0),
      });
    });
    seq.sort((a, b) => timeStrToMinutes(a.plannedTime) - timeStrToMinutes(b.plannedTime));

    const byField = { 1: [], 2: [] };
    seq.forEach((s) => byField[s.field].push(s));

    const byMatchId = new Map();
    const drift = { 1: { minutes: 0, isReal: false }, 2: { minutes: 0, isReal: false } };

    [1, 2].forEach((field) => {
      let runningMin = timeStrToMinutes(state.config.start);
      let anchorIsReal = false;

      byField[field].forEach((entry) => {
        const time = minutesToTimeStr(runningMin);
        byMatchId.set(entry.matchId, { time, isReal: anchorIsReal, plannedTime: entry.plannedTime });

        // Drift = Abstand zwischen dieser (ggf. echten) Kickoff-Zeit und der
        // ursprünglich geplanten — nur aussagekräftig, sobald wir für dieses
        // Spiel bereits einen echten Anker haben.
        if (anchorIsReal) {
          drift[field] = { minutes: timeStrToMinutes(time) - timeStrToMinutes(entry.plannedTime), isReal: true };
        }

        if (entry.match.finishedAt) {
          const fin = new Date(entry.match.finishedAt);
          runningMin = (fin.getHours() * 60 + fin.getMinutes()) + KICKOFF_LOCK_BUFFER_MINUTES;
          anchorIsReal = true;
        } else {
          runningMin += entry.durationMin;
        }
      });
    });

    return { byMatchId, drift };
  }

  // Kurzer, fertig formatierter Text fürs UI, z.B.
  // "Feld 1: 12 Min. hinter Zeitplan · Feld 2: 3 Min. vor Zeitplan".
  // Felder ohne echten Ankerpunkt (noch kein Ergebnis eingetragen) werden
  // weggelassen. Liefert "" wenn (noch) für kein Feld ein echter Anker existiert.
  function formatScheduleDriftText(drift, stadiumNames) {
    const parts = [1, 2].map((field) => {
      const d = drift && drift[field];
      if (!d || !d.isReal || d.minutes === 0) return null;
      const label = (stadiumNames && stadiumNames[field]) || `Feld ${field}`;
      const abs = Math.abs(Math.round(d.minutes));
      const dir = d.minutes > 0 ? "hinter" : "vor";
      return `${label}: ${abs} Min. ${dir} Zeitplan`;
    }).filter(Boolean);
    return parts.join(" · ");
  }
  ...
  global.MB = {
    ...
    computeLiveKickoffSchedule, formatScheduleDriftText,
